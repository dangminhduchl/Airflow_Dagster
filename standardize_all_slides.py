...
ACCENT_GREEN_HEX = "00E599" # #00E599 Cyber Teal/Green
...
def standardize_xml_elements(prs):
    # Map of colors to replace in slides 2, 3, 4, 5, 6
    fill_map = {
        "07050E": BG_NAVY,       # Replace dark violet bg with deep navy
        "000000": BG_NAVY,       # Replace pure black with deep navy
        "120E20": CARD_NAVY,     # Replace purple card with card navy
        "0F172A": SUBCARD_TEAL,  # Replace slate sub-card with teal dark
        "050814": CODE_NAVY,     # Code background
    }
    
    line_map = {
        "A855F7": ACCENT_CYAN_HEX,  # Purple neon -> Electric Cyan
        "C084FC": ACCENT_CYAN_HEX,  # Purple pastel -> Electric Cyan
        "818CF8": ACCENT_SKY_HEX,   # Indigo -> Sky Blue
        "581C87": ACCENT_CYAN_HEX,  # Dark Purple -> Electric Cyan
    }

    # Iterate slides 2 to 6 (indices 1 to 5)
    for idx in range(1, 6):
        slide = prs.slides[idx]
        
        # 1. Update solidFills
        for sp in slide.element.iter('{http://schemas.openxmlformats.org/drawingml/2006/main}solidFill'):
            clr = sp.find('{http://schemas.openxmlformats.org/drawingml/2006/main}srgbClr')
            if clr is not None:
                val = clr.get('val').upper()
                if val in fill_map:
                    clr.set('val', fill_map[val])

        # 2. Update lines
        for ln in slide.element.iter('{http://schemas.openxmlformats.org/drawingml/2006/main}ln'):
            # Update line color
            clr_el = ln.find('.//{http://schemas.openxmlformats.org/drawingml/2006/main}srgbClr')
            if clr_el is not None:
                val = clr_el.get('val').upper()
                if val in line_map:
                    clr_el.set('val', line_map[val])
            
            # Standardize line width: if width is > 20000 (1.8pt, 2.2pt, 3.0pt), cap to 19050 (1.5pt)
            w = ln.get('w')
            if w and int(w) > 20000:
...
def run():
    ...
    print("Standardizing Slide 7 to Cyber Teal / Deep Navy...")
    standardize_slide7(prs)
    
    print("Standardizing Slides 2-6 colors & borders...")
    standardize_xml_elements(prs)
    
    prs.save(path)
    print("Done! All slides standardized to Tone Xanh Navy / Cyber Teal with standard borders & corner radiuses.")
